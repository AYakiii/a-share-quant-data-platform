    ...
    *,
    ...
    def _path(raw_root, p):
        return (
            raw_root
            / source_name
            / f"exchange={p.values['exchange']}"
            / f"trade_date={p.values['trade_date']}"
            / "data.parquet"
        )
...
def test_max_workers_1_sequential_behavior(tmp_path, monkeypatch):
    ...
    RawWarehouseRunner(
        _make_spec(_fetch_ok, plan=plan),
        tmp_path / "raw",
        tmp_path / "out",
        "seq",
        max_workers=1,
...
def test_max_workers_2_parallel_and_artifacts(tmp_path, monkeypatch):
    ...
    manifest = json.loads(
        (run_dir / "warehouse_manifest.json").read_text(encoding="utf-8")
    )
...
def test_stock_inventory_symbol_preserves_leading_zero_after_read_csv(
    ...
    def _path(raw_root, p):
        return (
            ...
            / "stock_zh_a_daily"
            / "v1"
            / f"symbol={p.values['symbol']}"
            / f"start_date={p.values['start_date']}_end_date={p.values['end_date']}"
            / "data.parquet"
        )
...
def test_heartbeat_not_printed_when_show_progress_false(
...
def test_partition_batching_processes_all_partitions(tmp_path, monkeypatch):
    plan = lambda **kwargs: [
        FetchPartition(values={"exchange": "SSE", "trade_date": f"2025-01-0{i}"})
        for i in range(2, 8)
    ]
...
def test_dual_api_metadata_and_date_filtering(tmp_path, monkeypatch):
    ...
    def _hist(**kwargs):
        return type(
            ...
            (),
            {
                "raw": pd.DataFrame(
                    {
                        "date": ["2025-12-31", "2026-01-02", "2027-01-01"],
                        "x": [1, 2, 3],
                    }
                )
            },
    ...
    assert out["rows_before_filter"] == 3
    assert out["rows_after_filter"] == 1


def test_build_raw_warehouse_help_import_smoke():
    proc = subprocess.run(
        [sys.executable, "-m", "qsys.utils.build_raw_warehouse", "--help"],
        capture_output=True,
        text=True,
