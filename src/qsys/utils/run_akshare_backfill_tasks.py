...
from qsys.data.factor_lake.backfill_execute import akshare_execute_backfill_tasks
from qsys.data.factor_lake.backfill_tasks import filter_tasks, generate_tasks_from_default_backfill_plan


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run/dry-run AkShare Raw Factor Lake backfill tasks")
...
def main(argv: list[str] | None = None, *, runner: Callable[..., dict[str, Any]] = akshare_execute_backfill_tasks) -> int:
    ...
    result = runner(tasks, output_root=args.output_root, metastore_path=args.metastore_path, max_tasks=args.max_tasks, dry_run=dry_run, request_sleep=args.request_sleep)
